...
const CELLS_HORIZONTAL = 15
const CELLS_VERTICAL = 15
...
const GRID = Array(CELLS_VERTICAL).fill(null).map(() => Array(CELLS_HORIZONTAL).fill(false))
const VERTICALS = Array(CELLS_VERTICAL).fill(null).map(() => Array(CELLS_HORIZONTAL - 1).fill(false))
const HORIZONTALS = Array(CELLS_VERTICAL - 1).fill(null).map(() => Array(CELLS_HORIZONTAL).fill(false))
...
const stepThroughCell = (row, column) => {
    ...
    for (let neighbor of neighbors) {
        ...
        if (nextRow < 0 || nextRow >= CELLS_VERTICAL || nextColumn < 0 || nextColumn >= CELLS_HORIZONTAL) {
            continue
        } 
...
HORIZONTALS.forEach((row, rowIndex) => {
    row.forEach((open, columnIndex) => {
        ...
        const wall = Bodies.rectangle(
            columnIndex * UNIT_LENGTH_X + UNIT_LENGTH_X / 2, rowIndex * UNIT_LENGTH_Y + UNIT_LENGTH_Y, UNIT_LENGTH_X, 5, {
...
VERTICALS.forEach((row, rowIndex) => {
    row.forEach((open, columnIndex) => {
        ...
        const wall = Bodies.rectangle(
            columnIndex * UNIT_LENGTH_X + UNIT_LENGTH_X, rowIndex * UNIT_LENGTH_Y + UNIT_LENGTH_Y / 2, 5, UNIT_LENGTH_Y, 
            { isStatic: true, label: 'wall', render: {fillStyle: '#ecf0f1'}}
            )
        World.add(world, wall)
    })
...
const GOAL = Bodies.rectangle(
    WIDTH - UNIT_LENGTH_X / 2, HEIGHT - UNIT_LENGTH_Y / 2, UNIT_LENGTH_X * 0.7, UNIT_LENGTH_Y * 0.7, {
        isStatic: true,
        label: "goal",
        render: {
            fillStyle: '#2ecc71'
...
const BALL = Bodies.circle(
    UNIT_LENGTH_X / 2, UNIT_LENGTH_Y / 2, BALL_RADIUS, {
        label: 'ball'
    }
)
